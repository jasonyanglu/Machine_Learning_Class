%% Gussian Discriminant Analysis（协方差是相等的）
...
total_num1 = size(find(y==1),1);%y=1的个数
...
sigma1 = eye(size(H1,2));  %列数
sigma2 = eye(size(H2,2));

u11=repmat(u1,size(H1,1),1);
u22=repmat(u2,size(H2,1),1);

%G1=exp(-0.5*((H1-u11)*inv(sigma1)*(H1-u11)'))/((2*pi)*sqrt(det(sigma1)));%两维d/2=1
